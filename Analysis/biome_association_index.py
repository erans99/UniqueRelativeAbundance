...
import kernel_utils
from fiesta.albi.albi_lib import fiesta_lib
# Fiesta paper - https://link.springer.com/chapter/10.1007/978-3-319-56970-3_15
# repository - https://github.com/cozygene/albi
import datetime
...
def _parse_GCTA_results(stdout, phenotype_name):
    ...
    for line in stdout.split('\n'):
        ...
        if len(line_split) == 0:
            continue
        if line_split[0] == 'mean':
            intercept = float(line_split[1])
        if line_split[0][:2] == 'X_':
            fixed_effects.append(float(line_split[1]))
        if line_split[0] == 'V(G)':
            sig2g = float(line_split[1])
        if line_split[0] == 'V(e)':
            sig2e = float(line_split[1])
        if 'error' in line.lower():
            print ('GCTA', phenotype_name, line)
            return intercept, sig2g, sig2e, h2_reml, h2_reml_std, h2_reml_l, h2_reml_l_std, fixed_effects
        if line.startswith('V(G)/Vp') and 'V(G)/Vp_L' not in line:
            h2_reml, h2_reml_std = float(line_split[1]), float(line_split[2])
        if line.startswith('V(G)/Vp_L'):
            h2_reml_l, h2_reml_l_std = float(line_split[1]), float(line_split[2])
    return intercept, sig2g, sig2e, h2_reml, h2_reml_std, h2_reml_l, h2_reml_l_std, fixed_effects
...
def run_biome_association_index(phenotype_name):
    ...
    if np.sum(is_outlier) > 0:
        df_pheno.loc[is_outlier, phenotype_name] = np.nan
    ...
    intercept, sig2g, sig2e, h2_reml, h2_reml_std, h2_reml_l, h2_reml_l_std, fixed_effects = \
        _parse_GCTA_results(stdout, phenotype_name)
    ...
    s = s[ind]
    U = U[:, ind]
    CI = fiesta_lib.calculate_cis_general(np.array([h2_reml]), s, U, covariates, iterations=100, alpha=0.05,
                                          tau=0.4, use_convergence_criterion=False, use_progress_bar=False)
    line = '\t'.join([str(c) for c in [phenotype_name,
...
def prepare_data():
    ...
    for idx_cohort, obj in enumerate([objIL, objUS]):
        ...
        obj.loc[:, ['age', 'gender', 'client_id']].set_index('client_id').to_csv(
            os.path.join(output_dir, 'covariates__%s.csv' % cohort[idx_cohort]))
        obj.loc[:, ['hba1c', 'bmi', 'client_id']].set_index('client_id').to_csv(
            os.path.join(output_dir, 'hba1c_bmi__%s.csv' % cohort[idx_cohort]))
        obj.loc[:, [u'age', u'hba1c', u'bmi', u'bt__hdl_cholesterol', u'bt__cholesterol', u'bt__fasting_glucose',
                    u'bt__fasting_triglycerides', u'bowel_movement_frequency', u'bt__inr', u'bt__protein',
                    u'bt__sgot', u'bt__albumin', u'bt__alkaline_phosphatase', u'bt__bilirubin', u'bt__tsh',
                    u'currently_smokes', u'height', u'weight', u'client_id']].set_index('client_id').to_csv(
            os.path.join(output_dir, 'all_phenotypes__%s.csv' % cohort[idx_cohort]))
...
def run_LMM():
    ...
    if not os.path.exists(grm_file + '.grm.gz') or not os.path.exists(grm_file + '.grm.id'):
    ...
    for phenotype_name in df_pheno.columns:
        run_biome_association_index(phenotype_name)
    print("Finished!")
